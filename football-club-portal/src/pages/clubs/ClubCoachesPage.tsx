        ...
        <PageTitle
          ...
          subtitle={
            filteredCoaches.length !== allCoaches.length 
              ...
              : `View and manage all coaches at the club${allCoaches.filter(c => c.isArchived).length > 0 ? ` (${allCoaches.filter(c => c.isArchived).length} archived)` : ''}`
          }
          action={{
            label: '+ Add New Coach',
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {coachesByRole[role].map((coach) => (
                <div key={coach.id} className="relative group">
                  ...
                  <Link
                    to={Routes.coachSettings(clubId!, coach.id)}
                    className="absolute bottom-2 right-2 p-2 bg-white dark:bg-gray-700 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-gray-50 dark:hover:bg-gray-600"
                    title="Settings"
