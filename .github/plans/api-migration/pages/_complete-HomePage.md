...
- [x] Replace `currentUser` import with `getCurrentUser()` API call or create `useCurrentUser()` hook
- [x] Add loading state while user profile fetches
- [x] Add section-level skeleton placeholders for auth loading state (button area)
- [x] Add section-level skeleton placeholders for profile loading state (welcome message)
- [x] Add error handling for auth failures (redirect to login)
- [x] Ensure logged-out users do not see API error UI
- [x] Verify error card only appears when authenticated and profile fails
- [x] Update any references to `currentUser.clubs`, `currentUser.roles` to match API response shape
- [x] Remove import of `currentUser` from `@data/currentUser`
- [x] Test welcome message displays correctly with API data
- [x] Verify role-based dashboard content works with API user profile
- [x] Preserve all existing UI styling and layout
...
|---|---|---|
| `currentUser.firstName` | `UserProfile.firstName` | Direct mapping |
| `currentUser.lastName` | `UserProfile.lastName` | Direct mapping |
| `currentUser.email` | `UserProfile.email` | Direct mapping |
| `currentUser.role` | `UserProfile.role` | Direct mapping |
...
- The `UserProfile` type from the API has `firstName`, `lastName` fields matching the static data structure
