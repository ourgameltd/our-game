...
namespace OurGame.Application.UseCases.AgeGroups.Queries.GetReportCardsByAgeGroupId;

/// <summary>
/// Query to get all report cards for a specific age group
/// </summary>
public record GetReportCardsByAgeGroupIdQuery(Guid AgeGroupId) : IQuery<List<ClubReportCardDto>>;

/// <summary>
/// Handler for GetReportCardsByAgeGroupIdQuery
/// </summary>
public class GetReportCardsByAgeGroupIdHandler : IRequestHandler<GetReportCardsByAgeGroupIdQuery, List<ClubReportCardDto>>
{
    private readonly OurGameContext _db;

    public GetReportCardsByAgeGroupIdHandler(OurGameContext db)
    {
        _db = db;
    }

    public async Task<List<ClubReportCardDto>> Handle(GetReportCardsByAgeGroupIdQuery query, CancellationToken cancellationToken)
    {
        // Get all report cards for players belonging to this age group
        var sql = @"
            SELECT 
                pr.Id,
                pr.PlayerId,
        ...
        var reportData = await _db.Database
            .SqlQueryRaw<ReportCardRawDto>(sql, query.AgeGroupId)
