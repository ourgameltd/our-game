...
export default function DevelopmentPlanListCard({ plan, player, linkTo }: DevelopmentPlanListCardProps) {
  ...
  const overallProgress = totalGoals > 0 
    ...
    : 0;
  const indicatorClass = plan.status === 'active' ? 'bg-primary-500 dark:bg-primary-400' : 'bg-gray-300 dark:bg-gray-600';
        ...
        <div className="flex items-start gap-4 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-start justify-between gap-2 mb-1">
              ...
              <span className={`px-2 py-1 text-xs font-medium rounded-full capitalize shrink-0 ${getStatusColor(plan.status)}`}>
                {plan.status}
              </span>
            </div>
