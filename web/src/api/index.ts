/**
 * OurGame API
 * 
 * Export all API client functionality from a single entry point.
 * 
 * Usage:
 * ```typescript
 * import { apiClient, useMyTeams } from '@/api';
 * ```
 */

// Client and types
export { apiClient, getApiBaseUrl } from './client';
export type {
  ApiResponse,
  UpdateAgeGroupRequest,
  ArchiveAgeGroupRequest,
  UserProfile,
  UpdateCurrentUserRequest,
  TeamListItemDto,
  TeamColorsDto,
  TeamClubDto,
  TeamCoachDto,
  TeamPlayerDto,
  ClubDetailDto,
  ClubMediaLinkDto,
  ClubPublicMediaDto,
  ClubPublicMediaLinkDto,
  ClubColorsDto,
  ClubLocationDto,
  ClubStatisticsDto,
  MatchSummaryDto,
  MatchScoreDto,
  MatchDetailDto,
  PublishedMatchReportDto,
  MatchLineupDto,
  LineupPlayerDto,
  MatchReportDetailDto,
  GoalDetailDto,
  CardDetailDto,
  InjuryDetailDto,
  PerformanceRatingDto,
  MatchCoachDetailDto,
  MatchCoachRequest,
  MatchSubstitutionDetailDto,
  CreateMatchRequest,
  CreateMatchLineupRequest,
  CreateLineupPlayerRequest,
  CreateMatchReportRequest,
  CreateGoalRequest,
  CreateCardRequest,
  CreateInjuryRequest,
  CreatePerformanceRatingRequest,
  CreateMatchSubstitutionRequest,
  UpdateMatchRequest,
  UpdateMatchLineupRequest,
  UpdateLineupPlayerRequest,
  UpdateMatchReportRequest,
  UpdateGoalRequest,
  UpdateCardRequest,
  UpdateInjuryRequest,
  UpdatePerformanceRatingRequest,
  UpdateMatchSubstitutionRequest,
  AgeGroupListDto,
  AgeGroupDetailDto,
  AgeGroupStatisticsDto,
  AgeGroupPerformerDto,
  AgeGroupPlayerDto,
  AgeGroupCoachDto,
  AgeGroupCoachTeamDto,
  TeamWithStatsDto,
  TeamStatsDto,
  TeamOverviewDto,
  TeamOverviewTeamDto,
  TeamOverviewStatisticsDto,
  SystemFormationDto,
  SystemFormationPositionDto,
  TeamMatchSummaryDto,
  TeamMatchesDto,
  TeamMatchDto,
  TeamInfoDto,
  TeamPerformerDto,
  TeamTrainingSessionDto,
  ClubInfoDto,
  ClubPlayerDto,
  ClubPlayerAgeGroupDto,
  ClubPlayerTeamDto,
  ClubTeamDto,
  ClubTeamColorsDto,
  ClubCoachDto,
  ClubCoachTeamDto,
  CoachDetailDto,
  CoachTeamAssignmentDto,
  CoachAgeGroupCoordinatorDto,
  ClubTrainingSessionDto,
  ClubTrainingSessionsDto,
  TrainingSessionDetailDto,
  SessionDrillDetailDto,
  SessionAttendanceDetailDto,
  SessionCoachDetailDto,
  AppliedTemplateDetailDto,
  CreateTrainingSessionRequest,
  CreateSessionDrillRequest,
  CreateSessionAttendanceRequest,
  CreateAppliedTemplateRequest,
  CreateTrainingSessionCoachRequest,
  UpdateTrainingSessionRequest,
  UpdateSessionDrillRequest,
  UpdateSessionAttendanceRequest,
  UpdateTrainingSessionCoachRequest,
  ClubMatchDto,
  ClubMatchesDto,
  ClubKitDto,
  CreateClubKitRequest,
  UpdateClubKitRequest,
  TeamKitDto,
  TeamKitsDto,
  CreateTeamKitRequest,
  UpdateTeamKitRequest,
  AddPlayerToTeamRequest,
  AddPlayerToTeamResult,
  UpdateSquadNumberRequest,
  ClubReportCardDto,
  ClubReportCardPlayerDto,
  ClubReportCardPeriodDto,
  ClubReportCardDevelopmentActionDto,
  ClubReportCardSimilarProfessionalDto,
  TeamDevelopmentPlanDto,
  ReportCardDto,
  DevelopmentActionDto,
  SimilarProfessionalDto,
  TacticScopeDto,
  TacticListDto,
  TacticsByScopeResponseDto,
  TacticDetailDto,
  TacticDetailScopeDto,
  PositionOverrideDto,
  TacticPrincipleDto,
  CreateTacticRequest,
  UpdateTacticRequest,
  DrillLinkDto,
  DrillListDto,
  DrillsByScopeResponseDto,
  DrillDetailDto,
  DrillDetailLinkDto,
  DrillScopeDto,
  CreateDrillRequest,
  CreateDrillLinkRequest,
  CreateDrillScopeRequest,
  UpdateDrillRequest,
  UpdateDrillLinkRequest,
  DrillTemplateListDto,
  DrillTemplatesByScopeResponseDto,
  DrillTemplateDetailDto,
  CreateDrillTemplateRequest,
  UpdateDrillTemplateRequest,
  ArchiveDrillTemplateRequest,
  ClubDevelopmentPlanDto,
  ClubDevelopmentPlanPlayerDto,
  ClubDevelopmentPlanGoalDto,
  ClubDevelopmentPlanPeriodDto,
  AgeGroupDevelopmentPlanSummaryDto,
  AgeGroupDevelopmentPlanPlayerDto,
  AgeGroupDevelopmentPlanGoalSummaryDto,
  AgeGroupDevelopmentPlanPeriodDto,
  DevelopmentPlanDto,
  DevelopmentPlanDetailDto,
  DevelopmentPlanGoalDetailDto,
  DevelopmentGoalDto,
  CreateDevelopmentPlanRequest,
  CreateDevelopmentGoalRequest,
  UpdateDevelopmentPlanRequest,
  UpdateDevelopmentGoalRequest,
  PlayerDto,
  PlayerTeamMinimalDto,
  CreatePlayerRequest,
  UpdatePlayerRequest,
  PlayerAbilitiesDto,
  PlayerAttributesDto,
  PlayerAbilityEvaluationDto,
  EvaluationAttributeDto,
  CreatePlayerAbilityEvaluationRequest,
  UpdatePlayerAbilityEvaluationRequest,
  ArchivePlayerAbilityEvaluationRequest,
  PlayerRecentPerformanceDto,
  PlayerUpcomingMatchDto,
  CreateClubRequest,
  UpdateClubRequest,
  UpdateClubMediaLinkRequest,
  CreateTeamRequest,
  UpdateTeamRequest,
  UpdateCoachRequest,
  CreateCoachRequest,
} from './client';

// React hooks
export {
  useCurrentUser,
  useMyTeams,
  useTeamOverview,
  useTeamPlayers,
  useTeamCoaches,
  useTeamDevelopmentPlans,
  useTeamMatches,
  useTeamKits,
  useTeamReportCards,
  useCreateTeamKit,
  useUpdateTeamKit,
  useDeleteTeamKit,
  useTeamsByAgeGroupId,
  useMyChildren,
  useUpdateCurrentUser,
  useClubById,
  useClubPublicMedia,
  useClubStatistics,
  useAgeGroupsByClubId,
  useAgeGroupById,
  useAgeGroupStatistics,
  useAgeGroupPlayers,
  useClubPlayers,
  useClubTeams,
  useClubCoaches,
  useClubTrainingSessions,
  useTrainingSession,
  useCreateTrainingSession,
  useUpdateTrainingSession,
  useClubMatches,
  useClubKits,
  useCreateClubKit,
  useUpdateClubKit,
  useDeleteClubKit,
  useClubReportCards,
  useClubDevelopmentPlans,
  useSystemFormations,
  useAgeGroupDevelopmentPlans,
  useTacticsByScope,
  useTactic,
  useCreateTactic,
  useUpdateTactic,
  useDrillsByScope,
  useDrill,
  useCreateDrill,
  useUpdateDrill,
  useDrillTemplatesByScope,
  useDrillTemplateById,
  useCreateDrillTemplate,
  useUpdateDrillTemplate,
  useArchiveDrillTemplate,
  useCoach,
  usePlayer,
  usePlayerAbilities,
  useCreatePlayerAbilityEvaluation,
  useUpdatePlayerAbilityEvaluation,
  useDeletePlayerAbilityEvaluation,
  useArchivePlayerAbilityEvaluation,
  usePlayerRecentPerformances,
  usePlayerUpcomingMatches,
  useDevelopmentPlan,
  useMatch,
  useMatchReport,
  usePublishedMatchReport,
  useReportCard,
  useUpdateAgeGroup,
  useUpdatePlayer,
  useCreateClubPlayer,
  useUpdateClub,
  useCreateMatch,
  useUpdateMatch,
  usePublishMatchReport,
  useCreateTeam,
  useUpdateTeam,
  useUpdateCoach,
  useCreateCoach,
  useAddTeamPlayer,
  useRemoveTeamPlayer,
  useUpdateTeamPlayerSquadNumber,
  useAssignTeamCoach,
  useRemoveTeamCoach,
  useUpdateTeamCoachRole,
} from './hooks';
export type { ApiError, UseMutationState } from './hooks';

// User API
export { getCurrentUser } from './users';
export type { UserProfile as UserProfileType } from './users';
