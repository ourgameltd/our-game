...
import { useClubById, useClubDevelopmentPlans } from '@/api';
import type { ClubDevelopmentPlanDto, ClubDevelopmentPlanPlayerDto } from '@/api';
 ...
 */
function mapApiPlanToDevelopmentPlan(dto: ClubDevelopmentPlanDto): DevelopmentPlan {
  return {
    id: dto.id,
 ...
 */
function mapApiPlayerToPlayer(dto: ClubDevelopmentPlanPlayerDto): Player {
  return {
    id: dto.id,
    clubId: '',
...
export default function ClubDevelopmentPlansPage() {
  ...
  // Fetch club and development plans from API
  const { data: club, isLoading: clubLoading, error: clubError } = useClubById(clubId);
  const { data: plans, isLoading: plansLoading, error: plansError } = useClubDevelopmentPlans(clubId);

  const isLoading = clubLoading || plansLoading;
  const hasError = clubError || plansError;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-red-700 dark:text-red-300">
                  {clubError?.message || plansError?.message || 'An unexpected error occurred'}
                </p>
              </div>
            </div>
          </div>
      ...
      <main className="mx-auto px-4 py-4">
        ...
        {isLoading ? (
          ...
          <PageTitle 
            ...
            subtitle={`All player development plans across ${club?.name || 'Club'}`}
          />
        )}
